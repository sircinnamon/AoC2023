...
def count_from_start(starting_point):
	visited = {}
	to_check = deque([starting_point])
...
for i in range(len(grid)):
	# west side
	mx = max(mx, count_from_start(((0,i), "E")))
	# east side
	mx = max(mx, count_from_start(((len(grid[0])-1,i), "W")))

for i in range(len(grid[0])):
	# north side
	mx = max(mx, count_from_start(((i,0), "S")))
	# east side
	mx = max(mx, count_from_start(((i,len(grid)-1), "N")))
print(mx)